                  ...
                  <tr key={os.id} className="border-b border-[#222222] last:border-b-0 hover:bg-[#1a1a1a] transition-colors duration-[120ms]">
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex gap-1.5 justify-end">
                        <Link href={`/admin/ordens-servico/${os.id}`} className="px-2.5 py-1 rounded-[4px] text-xs font-medium border border-[#222222] text-[#888888] hover:border-[#f5c518] hover:text-[#f5c518] transition-colors duration-150">Editar</Link>
            ...
            {filtered.map((os) => (
              <div key={os.id} className="rounded-[8px] p-4 bg-[#111111] border border-[#222222] shadow-lg">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className="text-sm font-semibold text-[#f5c518] break-words">{os.numero}</span>
                    <p className="text-base font-medium mt-1 leading-tight break-words">{os.cliente}</p>
                  </div>
                ...
                <div className="flex flex-col gap-4 mt-4 pt-4 border-t border-[#1f1f1f]">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Link href={`/admin/ordens-servico/${os.id}`} className="flex justify-center items-center py-3 rounded-[6px] text-[14px] font-semibold border border-[#222222] bg-[#1a1a1a] text-[#888888] active:bg-[#222]">
                      Editar
                    </Link>
                    <button onClick={() => handleExportPDF(os.id)} className="flex justify-center items-center py-3 rounded-[6px] text-[14px] font-semibold border border-[#222222] bg-[#1a1a1a] text-[#888888] active:bg-[#222]">
                      Gerar PDF
                    </button>
                    <button onClick={() => handleDelete(os.id)} className="col-span-2 flex justify-center items-center py-3 rounded-[6px] text-[14px] font-semibold border border-[#e53e3e] bg-[rgba(229,62,62,0.05)] text-[#e53e3e] active:bg-[rgba(229,62,62,0.15)]">
                      Excluir
                    </button>
                  </div>
                </div>
