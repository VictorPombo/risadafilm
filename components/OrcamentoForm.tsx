          ...
          {itens.map((row, i) => (
            <div key={i} className="py-5 relative">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center bg-[#0a0a0a] p-3 rounded-[6px] border border-[#1f1f1f]">
                  <span className="text-[11px] uppercase tracking-wider text-[#888]">Total do Item</span>
                  <p className="text-right font-mono text-[15px] font-bold text-[#f5c518]">{formatCurrency(calcRowTotal(row))}</p>
                </div>
              </div>
            </div>
          ))}
